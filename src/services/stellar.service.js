...
const MARKET_KEY_MARKER_UP =
	"GA2UB7VXXXUSEAQUAXXXAQUARIUSVOTINGWALLETXXXPOWEREDBYAQUA";
const MARKET_KEY_MARKER_DOWN =
	"GAYVCXXXUSEAQUAXXXAQUARIUSDOWNVOTEWALLETXXXPOWEREDBYAQUA";
const MARKET_KEY_SIGNER_WEIGHT = 1;
const MARKET_KEY_THRESHOLD = 10;

export const DELEGATE_MARKER_KEY =
	"GA5BS7XXXAQUARIUSXXXICEXXXVOTEDELEGATIONXXXPOWEREDBYAQUA";

const AIRDROP_2_SPONSOR =
	"GDFCYDQOVJ2OEWPLEGIRQVAM3VTOQ6JDNLJTDZP5S5OGTEHM5CIWMYBH";

export const COLLECTOR_KEY =
	"GAORXNBAWRIOJ7HRMCTWW2MIB6PYWSC7OKHGIXWTJXYRTZRSHP356TW3";

export const StellarEvents = {
...
const { aquaCode, aquaIssuer, aquaAssetString } =
	ASSETS_ENV_DATA[getEnv()].aqua;
...
const AMM_REWARDS_KEY =
	"GC6ZWKVYRAUSCLMQYKDQZNTUNVDH2J5J6IELIWHMF7THRUQJQTQFQANA";
const SDEX_REWARDS_KEY =
	"GC5VEAWX7C3GSTW7RUKJKMQWXYZFW5TH4NGI4ZQSF6LNLUYSDGBVANBA";
const BRIBE_REWARDS_KEY =
	"GAORXNBAWRIOJ7HRMCTWW2MIB6PYWSC7OKHGIXWTJXYRTZRSHP356TW3";
...
export default class StellarServiceClass {
	...
	priceLumenUsd = null;
	claimableBalances = null;
	keypair = null;
	poolsRewardsNoteHash = new Map();
	...
	async buildTx(account, operations, memo) {
		const newAccount = await this.loadAccount(account.accountId());

		this.event.trigger({
			type: StellarEvents.handleAccountUpdate,
			account: newAccount,
		});
	...
	isMoreSignaturesNeeded(tx, account) {
		const { operations } = tx;
		...
		const transactionThreshold = operations.reduce((acc, operation) => {
			...
			return THRESHOLD_ORDER[usedThreshold] > THRESHOLD_ORDER[acc]
				? usedThreshold
				: acc;
		}, THRESHOLDS.LOW);
		...
		return masterKeyWeight < account.thresholds[transactionThreshold];
	}
	...
	resolveFederation(homeDomain, accountId) {
		return StellarSdk.StellarToml.Resolver.resolve(homeDomain)
			...
			.then((result) => result.data.stellar_address);
	}

	resolveToml(homeDomain) {
		return StellarSdk.StellarToml.Resolver.resolve(homeDomain);
	}

	startAccountStream(publicKey) {
	...
	getNextLocks(claims, next, limit) {
	...
	getNextClaimableBalances(next, limit) {
		next().then((res) => {
		...
		this.closeEffectsStream = this.server
			...
			.stream({
				onmessage: (res) => {
					...
					if (res.type === "account_debited") {
						const { amount, asset_type, asset_code } = res;
	...
	async getAccountOffers(publicKey) {
	...
	getMarketVotesValue(marketKey, accountId, asset) {
		if (!this.claimableBalances) {
			return null;
		}
	...
	getAirdrop2Claims() {
		...
		return this.claimableBalances.filter(
			(cb) => cb.sponsor === AIRDROP_2_SPONSOR
		);
	}

	getKeysSimilarToMarketKeys(accountId) {
		if (!this.claimableBalances) {
			return null;
		}
	...
	balancesHasChanges(prevBalances, newBalances) {
	...
	processIceTx(tx, asset) {
		...
		return axios
			.post(endpoint, { tx: tx.toEnvelope().toXDR("base64") })
			.then(({ data }) => {
	...
	async getLiquidityPoolForAccount(id, limit) {
		const { records, next } = await this.server
			.liquidityPools()
			.forAccount(id)
			.limit(limit)
	...
	createBurnAquaOperation(amount) {
	...
	async getAsset24hStats(base, counter) {
		...
		const volume = records.reduce(
			(acc, item) => acc + Number(item.base_volume),
			0
		);
		...
		const changes24h = (((lastPrice - startPrice) / startPrice) * 100).toFixed(
			2
		);
	...
	createClaimOperations(claimId, withTrust) {
		const ops = [];
	...
	getDelegateLocks(accountId) {
		...
		return this.claimableBalances
			...
			.map((cb) => {
				const unlockDate =
					cb.claimants.find(({ destination }) => destination === accountId)
						.predicate.not.abs_before_epoch * 1000;

				return { ...cb, unlockDate };
			});
	}

	getDelegatorLocks(accountId) {
		if (!this.claimableBalances) {
			return null;
		}
		...
		return this.claimableBalances.reduce((acc, claim) => {
			...
			const selfClaim = claim.claimants.find(
				(claimant) =>
					claimant.destination === accountId &&
					!!claimant.predicate?.not?.unconditional
			);
			const isUpvoteIce = claim.asset === `${UP_ICE_CODE}:${ICE_ISSUER}`;
		...
		return this.server
			...
			.then((res) => {
				...
				return res.records.reduce(function (prev, current) {
					return +prev.destination_amount > +current.destination_amount
						? prev
						: current;
				}).destination_amount;
			});
	...
	createWithdrawOperation(
		...
		const [assetA, assetB] = [base, counter].sort((a, b) =>
			StellarSdk.Asset.compare(a, b)
		);
	...
	getLumenUsdPrice() {
		return axios
			.get(`https://api.stellarterm.com/v1/ticker.json?${Math.random()}`)
			.then(({ data }) => data._meta.externalPrices.USD_XLM);
	}

	getLiquidityPoolData(base, counter) {
		return this.server
			.liquidityPools()
	...
	getPayments(accountId, limit = 200) {
	...
	encodePaymentParameterValue(value) {
		return StellarSdk.xdr.ScVal.fromXDR(value, "base64").value()?.toString?.();
	}

	isPaymentCalledMethod(parameters, methodName) {
	...
	loadNotes(payments) {
		chunkFunction(payments, (payment) => {
			if (
