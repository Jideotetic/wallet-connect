...
export function getAssetContractId(asset) {
	return asset.contractId(getNetworkPassphrase());
}

export function parseTokenContractId(contractId) {
	...
	if (
		contractId ===
		StellarService.createLumen().contractId(getNetworkPassphrase())
	) {
	...
	return buildSmartContractTx(
		...
		.then(({ result }) => {
			const [name, symbol, decimal] = result.retval
				.value()
...
export function getTokenDecimals(contactId) {
	return buildSmartContractTx(
		ACCOUNT_FOR_SIMULATE,
		contactId,
...
export async function getTokenBalance(token, where) {
	const tokenContact =
		typeof token === "string" ? token : getAssetContractId(token);
...
export function deployAssetContractTx(publicKey, asset) {
...
export function getTokenContractData(contractId) {
...
function getAssetContractHash(asset) {
	return new StellarSdk.Contract(asset.contract)
		.address()
		.toBuffer()
		.toString("hex");
}

export function orderTokens(assets) {
