...
export const BuildSignAndSubmitStatuses = {
	success: "success",
	pending: "pending",
};
...
export default class WalletConnectServiceClass {
	...
	async initWalletConnect(disconnectAll) {
		try {
			if (this.isOffline) {
	...
	listenWalletConnectEvents() {
		this.client.on(SIGN_CLIENT_EVENTS.session_delete, ({ topic }) => {
	...
	async connect(pairing, isAutoConnect) {
		if (this.isOffline) {
			ToastService.showErrorToast(INTERNET_CONNECTION_ERROR);
			return;
		}
	...
	async setClient() {
		...
		this.client = await Promise.race([
			WalletConnectClient.init({
				projectId: "b3caac6f2b5c23a9134c1a14d58509d9",
