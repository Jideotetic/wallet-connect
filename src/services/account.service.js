...
//             return WalletConnectService.signTx(tx);
//         }
...
//             return LedgerService.signTx(tx).then(res =>
...
//             return WalletConnectService.signAndSubmitTx(tx).then(res => {
//                 callCallbackIfExist();
//                 return res;
//             });
//         }
...
//             const result = LedgerService.signTx(tx )
//                 .then(signed =>
//                     withResult
...
//     getAssetBalance(asset, ignoreReserves)
//     getAssetBalance(asset, ignoreReserves) {
...
//             (lp  ).tokens = lp.reserves.map(reserve => {
...
//                 ...lp.reserves,
//             ];
//             (lp  ).total_share = new BigNumber(lp.total_shares)
//                 .times(1e7)
//                 .toFixed(7);
//             (lp  ).fee = 0.003;
//         });
...
//             balance =>
//                 (balance ).asset_code == aquaCode &&
//                 (balance ).asset_issuer === aquaIssuer,
//         )
...
//     async getReservesForSwap(asset) {
//         if (asset.type === TokenType.soroban) {
...
//         if (!asset.isNative()) {
//             const { selling_liabilities } = balance
...
import LedgerSignTx from "components/LedgerSignTx";
import SentToVault from "components/SentToVault";
import SignWithPublic from "components/SignWithPublic";
...
export default class AccountService extends Horizon.AccountResponse {
	...
	signTx(tx) {
		...
		if (this.authType === LoginTypes.lobstr) {
			return LobstrExtensionService.signTx(tx).then((res) =>
				res.toEnvelope().toXDR("base64")
			);
		}
		...
		if (this.authType === LoginTypes.walletKit) {
			return WalletKitService.signTx(tx).then((res) =>
				res.toEnvelope().toXDR("base64")
			);
		}
	}

	async signAndSubmitTx(tx, withResult, callback) {
		function callCallbackIfExist() {
			if (callback) {
				callback();
			}
	...
	getPoolBalance(id) {
		const poolBalance = this.balances.find(
			(balance) => balance.liquidity_pool_id === id
		);
	...
	async getClassicPools() {
		const liquidityPoolsForAccount =
			await StellarService.getLiquidityPoolForAccount(this.accountId(), 200);
		...
		liquidityPoolsForAccount.forEach((lp) => {
			lp.liquidity = lp.tokens
				.reduce((acc, asset, index) => {
					const price =
						prices.get(asset.contractId(getNetworkPassphrase()))?.price ?? 0;
		...
		return pools
			.map((pool) => {
				const lp = liquidityPoolsForAccount.find(
					({ id }) => id === pool.liquidity_pool_id
				);
	...
	getAquaBalance() {
		const aquaBalance = this.balances.find(
			(balance) =>
				balance.asset_code == aquaCode && balance.asset_issuer === aquaIssuer
		);
		...
		return new BigNumber(aquaBalance.balance)
			.minus(aquaBalance.selling_liabilities)
			.toNumber();
	}
	...
	getAvailableNativeBalance() {
		...
		const reserve =
			(2 + this.subentry_count + this.num_sponsoring - this.num_sponsored) *
			0.5;
	...
	async getSortedBalances() {
		...
		const classicBalances = classicAssetsBalances.map((balance) => {
			...
			return {
				balance: balance.balance,
				nativeBalance: nativePrices.has(contract)
					? +balance.balance * +(nativePrices.get(contract)?.price || 0)
					: 0,
				token: StellarService.createAsset(
					balance.asset_code,
					balance.asset_issuer
				),
			};
		...
		const customBalances = knownCustomTokens
			.map((token, index) => ({
				balance: customUserBalances[index],
				nativeBalance:
					+customUserBalances[index] *
					+(nativePrices.get(token.contract)?.price || 0),
				token,
			}))
			.filter(({ balance }) => !!Number(balance));
	...
	async getReservesForSwap(asset) {
		...
		const { entriesTrustlines, entriesLiquidityTrustlines } =
		...
		const items = [
			...
			{ label: "Trustlines", value: entriesTrustlines * 0.5 },
			{
				label: "My liquidity pool trustlines",
				value: entriesLiquidityTrustlines,
			},
			{ label: "Offers", value: entriesOffers * 0.5 },
			{ label: "Signers", value: entriesSigners * 0.5 },
			{ label: "Account data", value: entriesOthers * 0.5 },
