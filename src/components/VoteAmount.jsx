...
const VoteAmount = ({ pair, totalStats }) => {
	if (!pair.votes_value) {
		return null;
	}
	...
	const upIce =
		pair.extra.upvote_assets.find(
			({ asset }) => asset === `${UP_ICE_CODE}:${ICE_ISSUER}`
		)?.votes_sum ?? 0;
	const downIce =
		pair.extra.downvote_assets.find(
		...
		>
			...
			<Amount>
				<AmountRow>
					...
					<Percents>
						<PercentMobile $isBoosted={boosted}>
							{percentBoostedValue}%
						</PercentMobile>
					</Percents>
					<Info />
