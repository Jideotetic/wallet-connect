...
import DelegateBlockSmall from "./DelegateBlockSmall";
...
const MainPage = () => {
	...
	useEffect(() => {
		...
		if (
			params.has(UrlParams.sort) &&
			!Object.values(SortTypes).includes(params.get(UrlParams.sort))
		) {
	...
	const processAssetsFromPairs = useCallback(
		(pairs) => {
	...
	const changeSort = React.useCallback(
		(sortValue) => {
			...
			params.delete(UrlParams.base);
			params.delete(UrlParams.counter);
			navigate.push({ pathname: location.pathname, search: params.toString() });
			setPage(1);
		},
		[navigate, isLogged, location.pathname, location.search]
	);
	...
	}, [sort, account, processAssetsFromPairs]);
	...
	}, [claimUpdateId, account, pairs?.length, processAssetsFromPairs, sort]);

	useEffect(() => {
		if (sort === SortTypes.topVoted || sort === SortTypes.popular) {
			Promise.all([
				getPairsList(sort, PAGE_SIZE, page),
				getTotalVotingStats(),
	...
	}, [updateIndex, page, pairs, processAssetsFromPairs, sort]);

	useEffect(() => {
		if (
			!sort ||
			sort === SortTypes.yourVotes ||
			sort === SortTypes.withBribes
		) {
			return;
		}
	...
	}, [sort, page, processAssetsFromPairs]);
	...
	useEffect(() => {
		...
		setPairsLoading(true);
		getFilteredPairsList(searchBase, searchCounter, PAGE_SIZE, page).then(
			(result) => {
				setPairs(result.pairs);
				processAssetsFromPairs(result.pairs);
				setCount(result.count);
	...
	}, [searchBase, searchCounter, page, processAssetsFromPairs]);
	...
	}, [isLogged, account.authType]);

	const changeSearch = (assets) => {
	...
	const { totalUpIce, totalDIce, totalDownIce } = useMemo(() => {
		...
		return {
			totalUpIce: +assets.find(({ asset }) => asset === getAssetString(UP_ICE))
				?.votes_sum,
			totalDownIce: +assets.find(
				({ asset }) => asset === getAssetString(DOWN_ICE)
			)?.votes_sum,
			totalDIce: +assets.find(
				({ asset }) => asset === getAssetString(DELEGATE_ICE)
			)?.votes_sum,
		};
	}, [totalStats]);
	...
	const goToMarketPage = () => {
		navigate.push(
			`${MarketRoutes.main}/${
				searchBase.isNative()
					? "native"
					: `${searchBase.code}:${searchBase.issuer}`
			}/${
	...
	return (
		<MainBlock>
			<Background>
				...
				<Description>
					Lock your AQUA to mint ICE and vote on-chain. Your votes decide how
					AQUA emissions are distributed — and can earn you bribes and
			...
			<ExploreBlock $hasChosenPairs={hasChosenPairs}>
				...
				<Header ref={headerRef}>
					...
					/>
					{Boolean(
						pairs.length && pairs.some((pair) => Boolean(pair.timestamp))
					) && (
						<StatusUpdate>
							{totalStats ? (
								<Tooltip
				...
				{searchBase && searchCounter && !pairs.length && (
					<CreatePair onClick={() => goToMarketPage()}>
						<Market
							assets={[searchBase, searchCounter]}
