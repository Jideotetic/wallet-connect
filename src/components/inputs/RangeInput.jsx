...
const Pillar = styled.div.attrs(({ $value, $disabled }) => ({
	style: {
...
const Mark = styled.div.attrs(({ $percent, $value, $disabled }) => ({
	style: {
		background:
			!$disabled && $value >= $percent ? COLORS.titleText : COLORS.gray,
		border: `0.1rem solid ${
...
const Thumb = styled.div.attrs(({ $value, $isDrag }) => ({
	style: {
...
const CurrentValue = styled.div.attrs(({ $value }) => ({
	style: {
		left: `${$value}%`,
	},
}))`
	...
	color: ${({ $disabled }) =>
		$disabled ? COLORS.placeholder : COLORS.titleText};
`;

const getClickPosition = (event, ref) => {
