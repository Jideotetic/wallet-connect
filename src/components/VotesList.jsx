...
import { goToStellarExpert } from "constants/votes-list";
import { CellAlign } from "constants/table";
...
const VotesList = ({ votes, pair, withoutClaimDate }) => {
	...
	}, [account, selectedClaims, selectAll, pendingId, noPendingClaims]);
	...
	const onSubmit = useCallback(
		...
		[account, claims, isMounted, selectedClaims]
	);
	...
	const getActionBlock = useCallback(
			...
			return {
				children: (
					<Checkbox
						...
						disabled={
							new Date(claim.claimBackDate) > new Date() || Boolean(pendingId)
						}
					/>
				),
				align: CellAlign.Center,
		...
		[pendingId, account, selectedClaims, selectClaim, onSubmit]
	);
									...
									<Market
										assets={[
											StellarService.createAsset(
												pair?.asset1_code || claim.asset1_code,
												pair?.asset1_issuer || claim.asset1_issuer
											),
											StellarService.createAsset(
												pair?.asset2_code || claim.asset2_code,
												pair?.asset2_issuer || claim.asset2_issuer
											),
										]}
										withoutDomains
										withoutLink
