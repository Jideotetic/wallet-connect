...
const SignWithPublic = ({ params, confirm }) => {
	...
	useEffect(() => {
		...
		StellarService.resolveFederation(account.home_domain, accountId).then(
			(res) => {
				setFederation(res);
			}
		);
	}, [account.home_domain, accountId]);
			...
			<CopyStyled text={xdr} withoutLogo>
				<ActionContainer>
					...
					<ActionMain>
						...
						<ActionDescription>
							Copy transaction XDR and paste into Stellar Laboratory or other
							wallet
						</ActionDescription>
					</ActionMain>
					<CopyIcon />
