...
const VotesAmountModal = ({ params, close }) => {
	...
	const [targetAsset, setTargetAsset] = useState(
		asset ?? (isDownVoteModal ? DOWN_ICE : UP_ICE)
	);
	...
	const keyType = isDownVoteModal ? "downvote_account_id" : "market_key";
	...
	useEffect(() => {
		if (pairsAmounts) {
			const sum = Object.values(pairsAmounts).reduce((acc, value) => {
				acc += Number(value);
				return acc;
			}, 0);
			...
			const percentValue = roundToPrecision(
				(Number(sum) / Number(targetBalance)) * 100,
				1
			);

			setPercent(+percentValue);
		}
	}, [pairsAmounts, targetBalance]);
	...
	}, [selectedPairs, keyType]);

	useEffect(
		() => () => {
			resetForm();
		},
		[targetAsset, resetForm]
	);
	...
	const onInputChange = (value) => {
		...
		const percentValue = roundToPrecision(
			(Number(value) / Number(targetBalance)) * 100,
			1
		);
	...
	const onPairInputChange = (value, marketKey) => {
		...
		const sum = Object.values(newValue).reduce((acc, value) => {
			acc += Number(value);
			return acc;
		}, 0);
	...
	const deletePair = (deletedPair) => {
		const updatedPairs = selectedPairs.filter(
			(pair) => pair[keyType] !== deletedPair[keyType]
		);
		...
		if (isHandleEdit) {
			...
			const sum = Object.values(pairsAmountCopy).reduce((acc, value) => {
				acc += Number(value);
				return acc;
			}, 0);
		...
		setPairsAmount(
			updatedPairs.reduce((acc, pair) => {
				acc[pair[keyType]] = roundToPrecision(
					Number(amount) / updatedPairs.length,
					7
				);
				return acc;
			}, {})
	...
	const confirmVotes = async () => {
		try {
			...
			const voteOps = Object.entries(pairsAmount).map(
				([marketKey, voteAmount]) =>
		...
		<ModalWrapper $width="60rem">
			...
			<ContentRow>
				...
				{hasTrustLine ? (
					...
					<BalanceBlock>
						You don’t have {targetAsset.code} trustline
					</BalanceBlock>
				)}
			</ContentRow>
			...
			{hasTrustLine && hasTargetBalance ? null : (
				<GetAquaBlock>
					<GetAquaLabel>
						You don&apos;t have enough {targetAsset.code}
					</GetAquaLabel>
