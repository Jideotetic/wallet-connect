...
import { IconFail, IconSuccess } from "./Icons";
...
const ToastBody = styled.div`
	...
	${respondDown(Breakpoints.md)`
        ...
        animation: ${({ $isShow }) =>
					$isShow
						? "openingToast ease-in-out 0.8s"
						: "closingToastBottom linear 0.4s"};
        min-height: 8rem;
...
export const Toast = ({ text, resolver, delay, type }) => {
	...
	const transitionHandler = (animationName) => {
		if (
			!isShow &&
			(animationName === "closingToast" ||
				animationName === "closingToastBottom")
		) {
			resolver();
		}
	};
