...
const Label = styled.div`
	position: absolute;
	bottom: ${({ $isOpen }) =>
		$isOpen ? "calc(100% + 1.3rem)" : "calc(100% + 1.2rem)"};
...
const AssetDropdown = ({
	...
	const knownAssetsList = (customAssetsList || knownAssets).map((asset) => {
		if (asset.type) {
			return asset;
		}
	...
	const filteredBalances = customAssetsList
		? balances.filter((balance) =>
				knownAssetsList.find(
					(knownAsset) => knownAsset.contract === balance.token.contract
				)
		  )
		: balances;
					...
					return (
						...
						!excludeList?.find(
							(excludeToken) =>
								...
								item.token.issuer === excludeToken.issuer
						)
					);
				})
				.filter(({ token }) => {
			...
			>
				...
				{selectedAsset && !isOpen ? (
					...
					<StyledInput
						...
						placeholder={
							placeholder ??
							"Search asset or enter home domain or contract address"
						}
						...
						prefixCustom={
							<ChipsWeb assets={selectedAssets} resetAsset={resetAsset} />
						}
						ref={inputRef}
					/>
				)}
				...
				{isOpen && (
					<DropdownList $longListOnMobile={longListOnMobile}>
						...
						{!filteredAssets.length && (
							<SearchEmpty>Asset not found.</SearchEmpty>
						)}
					</DropdownList>
				)}
			</DropDown>
