...
export function processNewAssets(assets) {
	return (dispatch) => {
		const cached = new Map(
			JSON.parse(localStorage.getItem(ASSET_CACHE) || "[]")
		);
		...
		if (!cached.has(ALL_ICE_ASSETS[0])) {
			newAssets.push(...ALL_ICE_ASSETS.map((str) => getAssetFromString(str)));
		}
		...
		if (getIsTestnetEnv()) {
			...
			newAssets.forEach((asset) => {
				cached.set(`${asset.code}:${asset.issuer}`, {
					...asset,
					...{ image: null },
				});
			});
			localStorage.setItem(
				ASSET_CACHE,
				JSON.stringify(Array.from(cached.entries()))
			);
			dispatch({
