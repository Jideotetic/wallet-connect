export const getDateString = (timestamp, config) => {
	...
	return `${months[month]}${withoutDay ? "" : ` ${day}`}${
		withoutYear ? "" : `, ${year}`
	}${
		withTime
			? `, ${hours == 24 ? "00" : `0${hours}`.slice(-2)}:${`0${minutes}`.slice(
					-2
			  )}`
			: ""
	}`;
...
export const getTimeAgoValue = (timestamp) => {
...
export const getTimeString = (timestamp) => {
	...
	return `${hours == 24 ? "00" : `0${hours}`.slice(-2)}:${`0${minutes}`.slice(
		-2
	)}`;
};

export const convertLocalDateToUTCIgnoringTimezone = (utcDate) =>
	new Date(
...
export function convertUTCToLocalDateIgnoringTimezone(date) {
