...
const hasWalletConnectDB = async () => {
	try {
...
const openWalletConnectDB = async () => {
...
const getFromWalletConnectDB = async (key) => {
	try {
		const db = await openWalletConnectDB();

		const store = db
			.transaction(WC_STORE_NAME, "readonly")
			.objectStore(WC_STORE_NAME);
...
export const sessionExistsInStorage = async () => {
	const sessionList = JSON.parse(
		(await getFromWalletConnectDB(WC_SESSION_ALIAS)) || "[]"
	);
...
export const savePairingToDeepLinkHistory = (topic) => {
	const currentWallet = getCurrentWallet();
