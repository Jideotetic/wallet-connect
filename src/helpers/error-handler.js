...
const TRANSACTIONS_ERROR_CODES = {
	// TODO: Add this codes
	...
	tx_bad_seq:
		"Transaction failed because sequence got out of sync. Please reload and try again.",
...
const OPERATIONS_ERROR_CODES = {
	...
	op_cross_self:
		"This path payment would cross one of its own offers or the account has opposite offer of equal or lesser price active, so the account creating this offer would immediately cross itself.",
	op_over_source_max:
...
export const KnownPrepareErrors = {
	// TODO: Add more codes
	"Error(Contract, #205)":
		"Depositing is currently disabled for this pool. Please reach out to support.",
	"Error(Contract, #206)":
		"Swapping is currently disabled for this pool. Please reach out to support.",
	"Error(Contract, #207)":
		"Claiming is currently disabled for this pool. Please reach out to support.",
...
export default function ErrorHandler(error) {
	...
	if (
		...
		error?.message === "The user rejected this request." // Freighter
	) {
...
export function SorobanPrepareTxErrorHandler(error) {
	const code = findErrorCode(error);
	return KnownPrepareErrors[code] ?? "Oops. Something went wrong.";
}

export function SorobanErrorHandler(errorName) {
	const snackCaseName = errorName.replace(
		/[A-Z]/g,
		(letter) => `_${letter.toLowerCase()}`
	);

	return (
		TRANSACTIONS_ERROR_CODES[snackCaseName] ?? "Oops. Something went wrong."
	);
}
